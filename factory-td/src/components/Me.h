#pragma once
// =====================================================================
// Me.h —— AE2 式存储物流网络组件（后期科技）
//
// 前期用物品管道（Pipe，EnderIO/Pipez 式逐段运输）；
// 后期用 ME 网络（AE2 式）：所有物品作为"数据"存入网络，全网共享。
//
//   方块          | 作用
//   MeInterface  | 网络桥接器：从相邻管道/分流器"吸入"物品入网；
//                | 向相邻机器/桶/塔"导出"物品出网（输入总线+输出总线合体）
//   MeDrive      | 存储单元：为网络提供存储容量（AE2 ME驱动器）
//   MeTerminal   | 终端：右键查看全网物品（AE2 ME终端）
//
...
/// ME接口（桥接物理世界与网络）
struct MeInterface {
    uint8_t connMask = 0;    // bit d: 该方向相邻是ME设备
