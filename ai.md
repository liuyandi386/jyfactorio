...
**K. 新手教程补全「组装机 → 弹药 → 炮塔」工序（本轮修复，代码已改，未发版）**
...
1. **人工实机走查（✅ 已完成 2026-09-15，用户实机确认）** —— 下列清单**保留作为回归 / 改版后的复测参考**，已不是待办遗留：
  ...
  - **新手教程 · 弹药工序（v1.3.5 改版后重点复测）**：确认新增的「造出第一批弹药 → 给组装机供料 → 架起防线 → 打通弹药线 → 首次击杀」能**一路走通且炮塔真的开火**（这正是本次修掉的死锁点，**只能人眼确认**）；顺带确认旧 `saves/tutorial.json` 因 `script_v` 不匹配被丢弃、从第 1 步重开（预期行为，不应报错）。
...
2. **新手教程**：**工序断层（缺"组装机 → 弹药"）已修，见 §0.1-K**。仍可选（先问用户）：文案/章节划分是否合适？是否需要"教程内可跳过到任意章节"？
